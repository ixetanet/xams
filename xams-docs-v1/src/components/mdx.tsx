...
export function Note({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-6 flex gap-2.5 rounded-2xl border border-green-500/20 bg-green-50/50 p-4 leading-6 text-green-900 dark:border-green-500/30 dark:bg-green-500/5 dark:text-green-200 dark:[--tw-prose-links-hover:theme(colors.green.300)] dark:[--tw-prose-links:theme(colors.white)]">
      <InfoIcon className="mt-1 h-4 w-4 flex-none fill-green-500 stroke-white dark:fill-green-200/20 dark:stroke-green-200" />
...
export const DocImage = (props: { image: StaticImageData; full?: boolean }) => {
  return (
    <div className="flex w-full justify-center">
      <Image
        src={props.image}
        alt=""
        className={`w-full ${props.full ? `w-full` : `sm:w-[32rem]`}`}
        // className="w-full"
        unoptimized
      />
    </div>
