...
Service classes can be added to the Project.Services/ServiceLogic folder. {{ className: 'lead' }}

## Logic Stage

Service Logic can execute at the listed stages below.

- PreValidation: Before security validation has occurred
...
```c# {{ title: 'Project.Services / ServiceLogic / MyEntityService.cs' }}
...
- LogicStages: Flags that determine which stages the service logic should exeucte on, PreValidation, PreOperation, or PostOperation.
...
In order to ensure that ServiceLogic is run on Create, Update, or Delete of a record from within a ServiceLogic class, use the DataService on the ServiceContext.
