...
```c# {{ title: 'Project / Actions / MyAction.cs' }}
...
Actions support constructor-based dependency injection just like Service Logic. The framework uses `ActivatorUtilities.CreateInstance()` to instantiate action classes, allowing automatic injection of registered services:
...
public class MyAction : IServiceAction
{
    ...
    public MyAction(IEmailService emailService, ICacheService cacheService)
    {
    ...
    public async Task<Response<object?>> Execute(ActionServiceContext context)
    {
        ...
        await _emailService.SendNotification($"Action executed with param: {parameters.MyParam1}");
        await _cacheService.InvalidateCache("my-cache-key");
        
        // Use ActionServiceContext for framework services
        context.Logger.LogInformation("MyAction executed successfully");
        var db = context.GetDbContext<DataContext>();

        return ServiceResult.Success(new { message = "Action completed" });
    }
...
- **ActionServiceContext**: Framework services like `context.Logger`, `context.GetDbContext<T>()`, `context.ExecutingUserId`, `context.File`, etc.
