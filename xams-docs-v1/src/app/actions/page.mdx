...
```c# {{ title: 'Project.Services / ServiceActions / MyAction.cs' }}
...
public class MyAction : IServiceAction
{
    public async Task<Response<object?>> Execute(ServiceContext context)
    {
        if (!context.Parameters.ContainsKey("myParam1") || !context.Parameters.ContainsKey("myParam2"))
        {
            return ServiceResult.Error("Missing Parameters");
        }
        string myParam1 = context.Parameters["myParam1"].GetString();
        int myParam2 = context.Parameters["myParam2"].GetInt32();
...
The file can be accessed in the Action using the File property on the ServiceContext.
