...
n React, you can check if a user has any or all permissions from an array by using the `hasAnyPermissions` or `hasAllPermissions` methods from the `useAuthRequest` hook.
...
const MyComponent = () => {
  ...
  const checkPermissions = useCallback(async () => {
    const hasAnyPermissions = await authRequest.hasAllPermissions([
