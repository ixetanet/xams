            ...
            foreach (IDictionary<string, object?> rootRecord in results)
            {
                foreach (Join join in readInput.joins)
                {
                    ...
                    if (join.fromTable == fromTable)
                    {
                        ...
                        {
                            if (!aRecord.ContainsKey(join.toField))
                            {
                                continue;
                            }

                            if (!rootRecord.TryGetValue(join.fromField, out var rootRecordValue))
                            {
                                continue;
                            }
                            
                            if (aRecord[join.toField] == null || rootRecordValue == null)
                            {
                                continue;
                            }
                            ...
                            {
                                throw new Exception($"Failed to denormalize join {join.fromTable}.{join.fromField} to {alias}.{join.toField} - {e.Message}");
                            }
                        }
