...
/// </summary>
[JobInitialState(JobState.Inactive)]
[ServiceJob(nameof(ImportDataJob), "ImportDataJob", "00:00:00", JobSchedule.Interval, DaysOfWeek.All, "System")]
public class ImportDataJob : IServiceJob
{
    public async Task<Response<object?>> Execute(JobServiceContext context)
    {
        if (!context.Parameters.TryGetValue("FilePath", out var parameter))
        {
            return ServiceResult.Error("Missing parameter: FilePath");
        }
        ...
        try
        {
            ...
            ADMIN_ExportData.TableExport[]? imports =
                JsonSerializer.Deserialize<ADMIN_ExportData.TableExport[]>(content);
            if (imports == null)
            {
                    ...
                    foreach (var item in ((JsonElement)import.data).EnumerateArray())
                    {
                                                       ...
                                                       && x.PermissionId == entity.GetValue<Guid>("PermissionId")))
                        {
                            var updateEntity = existing
                                .FirstOrDefault(x =>
                                    x.GetValue<Guid>("RolePermissionId") == entity.GetValue<Guid>("RolePermissionId"));
