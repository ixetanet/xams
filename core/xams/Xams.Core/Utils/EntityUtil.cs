...
namespace Xams.Core.Utils
{
    public static class EntityUtil
    {
        ...
        public static object DictionaryToEntity(Type type, Dictionary<string, dynamic> fields)
        {
            ...
            foreach (var property in type.GetEntityProperties())
            {
                if (fields.ContainsKey(property.Name))
                {
                    ...
                    {
                        property.SetValue(entity, Convert.ToString(fields[property.Name]));
                    }
                    else
                    {
        ...
        public static bool ValueChanged(object? ent, object? preEntity, string fieldName)
        {
            if (ent == null) throw new Exception("Entity is null. Cannot check if value changed.");
            var entity = ent;
